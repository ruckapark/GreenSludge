...
if __name__ == "__main__":
    ...
    for conc in data.concs:
        ...
        dataset_scores[conc] = {
            'changes': changes[conc],
            'activity': activity[conc],
            'activity_left': activity_left[conc],
            'activity_right': activity_right[conc],
            'preferred_zone': pref_zone[conc],
            'end_zone': end_zone[conc],
            'zone_time': zonetime[conc],
            'max_amplitude_left': maxampG[conc],
            'max_amplitude_right': maxampD[conc],
            'mean_amplitude_left': meanampG[conc],
            'mean_amplitude_right': meanampD[conc]
        }
    
    # calculate scores for each concentrations
    vdt.write_res(
        dataset_scores,
        data.site,
        data.batch,
