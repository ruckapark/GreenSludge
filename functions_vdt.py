...
def find_changes(ser, window=5, fps=15):
    ...
    changes = pd.Series(
        np.array(signs)[1:] - np.array(signs)[:-1], index=signs.index[:-1]
    )
...
def adjust_starts(df, start_times, active_cols, time=10, fps=15):
    ...
    final_df = pd.DataFrame(
        index=np.linspace(0, time * 60000, time * 60 * fps + 1, dtype=int),
        columns=active_cols,
    )
    for i in range(20):
        if "G_{}".format(i + 1) not in active_cols:
            continue
...
def adjust_starts_old(df, end_times, active_cols):
    ...
    final_df = pd.DataFrame(
        index=np.linspace(0, 900000, 4501, dtype=int), columns=active_cols
    )
    for i in range(20):
        if "G_{}".format(i + 1) not in active_cols:
            continue
...
def plotgraphs(df, active_cols, title="No known concentration"):
    """Plot all individual trajectories of vers de terre"""
    df.index /= 1000 * 60  # convert index to minutes

    # create figure with shared x and y axes, loop through active organisms and plot
    fig, ax = plt.subplots(4, 5, figsize=(19.2, 10.8), sharex=True, sharey=True)
    for i in range(20):
        if "G_{}".format(i + 1) not in active_cols:
            continue
        row, col = i // 5, i % 5
        ax[row, col].plot(-df["G_{}".format(i + 1)], color="r")  # left side (humide)
        ax[row, col].plot(df["D_{}".format(i + 1)], color="b")  # right side (sec / dry)
...
def create_csv_file(file_path):
    ...
    if not os.path.exists(file_path):
        ...
        with open(file_path, "w", newline="") as file:
            ...
            header = ["Site", "Date", "Date_test", "Concentration", "Measure"] + [
...
def single_frame(df, measure):
    ...
    temp = df[df["Measure"] == measure].drop(
        columns=["Date_test", "Measure", "Site", "Date"]
    )
